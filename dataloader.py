...
class TrainPerson(Dataset):
	...
	def __getitem__(self, idx):
		if idx < len(self.list_dir("imgs/0")) - TIMESTEPS:
			...
			for time in range(TIMESTEPS):
				search = Image.open("../rec_siam_unet/pathignore/data/train/imgs/0/" + names[idx+time]).convert('RGB')
				search = self.search_trans(search).unsqueeze(dim=0)
				searchs = torch.cat([searchs, search], dim=0)
				mask = Image.open("../rec_siam_unet/pathignore/data/train/anno/0/" + names[idx+time][:5] + ".png").convert('L')
			...
			masks = torch.tensor([])
			for time in range(TIMESTEPS):
				search = Image.open("../rec_siam_unet/pathignore/data/train/imgs/1/" + names[idx-67+time]).convert('RGB')
				search = self.search_trans(search).unsqueeze(dim=0)
				searchs = torch.cat([searchs, search], dim=0)
				mask = Image.open("../rec_siam_unet/pathignore/data/train/anno/1/" + names[idx-67+time][:5] + ".png").convert('L')
				mask = self.search_trans(mask).unsqueeze(dim=0)
				masks = torch.cat([masks, mask], dim=0)
	...
	def __len__(self):
		path_0 = len(self.list_dir("imgs/0")) - TIMESTEPS + 1
		path_1 = len(self.list_dir("imgs/1")) - TIMESTEPS + 1
...
if __name__ == '__main__':
	...
	inp = int(input())
	target, searchs, labels, depths, score_labels = dataset[inp]
